...
import '../main.dart';
import '../modals/SignUp_model.dart';
...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  createAccount() async {
    if (path.isEmpty) {
      ...
      print("Sign Up Parameter ${formData}");
      print(SERVER_ADDRESS + "/api/userregister");
      response =
          await dio.post(SERVER_ADDRESS + "/api/userregister", data: formData);
      if (response.statusCode == 200) {

        signUpModel = SignUpModel.fromJson(jsonDecode(jsonEncode(response.data)));
        print('${response.data['status']}____________________');
