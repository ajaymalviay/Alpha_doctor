...
import 'package:complete_women_care/chatModule/MyPhotoViewer.dart';
import 'package:complete_women_care/chatModule/MyVideoPlayer.dart';
import 'package:complete_women_care/chatModule/MyVideoThumbnail.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver{
  ...
  final DatabaseReference databaseReference2 = FirebaseDatabase.instance.reference();
  final DatabaseReference databaseReference3 = FirebaseDatabase.instance.reference();
  ...
  void initState() {
    ...
    _lvScrollCtrl.addListener(() {
      ...
      if(_lvScrollCtrl.position.maxScrollExtent  == _lvScrollCtrl.position.pixels){
        setState(() {
  ...
  String getSendMessageTime(){
    ...
    Duration duration = DateTime.now().difference(DateTime.parse(currentTime.toString()));
    return DateTime.parse(currentTime.toString()).add(duration).toUtc().toString();
  }

  // loadUserProfile() async{
  //   await FirebaseDatabase.instance.reference().child(widget.uid.toString()).once().then((value){
  //     print("loading "+widget.uid.toString());
  //     setState(() {
  //       imageLink = SERVER_ADDRESS+"/public/upload/"+value.value['profile'].toString();
  //       print("loaduserprofile" + SERVER_ADDRESS+"/public/upload/"+value.value['profile'].toString());
  //     });
  //   }).catchError((e){
  //
  //   });
  //
  //   // await FirebaseDatabase.instance.reference().child(widget.uid).child("TokenList").once().then((value){
  ...
  //
  //   // await FirebaseDatabase.instance.reference().child(myUid).once().then((value){
  //   //   if(mounted){
  //   //     setState(() {
  ...
  //   // });
  //   //
  //   // await FirebaseDatabase.instance.reference().child(myUid).child("chatlist").child(widget.uid).once().then((value){
  //   //   print("----------> "+value.value.toString());
  //   //   if(value.value != null) {
  //   //     if(mounted){
  //   //       setState(() {
  ...
  uploadDataWithBackgroundService(String taskId, result) async{
    ...
    dbRef.once().then((value) async {
      ...
      if (isFirstMessage) {
        ...
        DatabaseReference dbRef2 = FirebaseDatabase.instance.reference()
            .child(widget.uid)
            .child("chatlist")
            .child(myUid);
        ...
        //     "type": jsonDecode(result.response)['data'].toString().contains(
        //         ".jpg") ? 1 : 2,
        //     "messageCount": value.value == null ? 1 : value
  ...
  getUserPreference() {
    ds = databaseReference2.child(widget.uid.toString()).onValue.listen((event) {
      ...
      //         userStatus = "last seen ${DateTime.parse(event.snapshot.value['last_seen'] + 'Z').toLocal().toString().substring(0, 19)}";
      //       }
      //     }
      //   }
      // });
    ...
    databaseReference3.child(myUid.toString()).child("chatlist").child(widget.uid).child("typing").onValue.listen((event) {
  ...
  void markAsTyping() {
    ...
    db.child(widget.uid).child("chatlist").child(myUid).child("typing").onValue.listen((event) {
      ...
      child: Scaffold(
        ...
        body: requestStatus == null
          ...
          children: [
            header(
                widget.userName,
                showLoading ? "Loading..." : isTyping ? "Typing..." : userStatus,
                imageLink
            ),
            SizedBox(height: 5,),
            ...
            Expanded(
              flex: 1,
              child: StreamBuilder(
                  stream: FirebaseFirestore.instance.collection("Chats/$channelId/All Chat")
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if(index == snapshot.data.docs.length){
                              if(snapshot.connectionState == ConnectionState.waiting && snapshot.data.docs.length> 20) {
                                print("\n\nwaiting : loaded");

                                return Container();
                              }else if(snapshot.connectionState == ConnectionState.active && snapshot.data.docs.length> 20) {
                                print("\n\nactive : Loading " + snapshot.hasData.toString());
                                isLoading = true;
                              ...
                              lastMessageUid = snapshot.data.docs[index]['uid'];
                              lastMessageDate = snapshot.data.docs[index]['time'].toString();



                              int k = snapshot.data.docs.length > index ? index + 1 : snapshot.data.docs.length - 1;
                              print("kkkkkkk $k");
                              print("hhhh " +snapshot.data.docs[index]['time'].toString());


                              int daysDifference = DateTime.now().difference(DateTime.parse(snapshot.data.docs[index]['time'].toString()+"Z").toLocal()).inDays;
                              int daysDifference2 = k >= snapshot.data.docs.length ? 3 :DateTime.now().difference(DateTime.parse(snapshot.data.docs[index]['time'].toString()+"Z").toLocal()).inDays;
                              print("days difference : $daysDifference days difference 2 : $daysDifference2");
                              print("snapshot.data.docs ${snapshot.data.docs}");

                              return Column(
                                children: [
                                  k >= snapshot.data.docs.length
                                      ? (daysDifference2 - daysDifference) >= 1 ? Row(
                                    children: [
                                      Expanded(
                                        child: Divider(
                                          height: 10,
                                      ...
                                      SizedBox(width: 10,),
                                      Text(daysDifference == 0 ? "Today" : daysDifference == 1 ? "Yesterday" :"${DateTime.now().add(Duration(days: -daysDifference)).day}  ${monthsList[DateTime.now().add(Duration(days: -daysDifference)).month - 1]}, ${DateTime.now().add(Duration(days: -daysDifference)).year}",
                                        style: TextStyle(
                                            fontSize: 12,
                                            color: LIGHT_GREY_TEXT
                                        ),
                                      ...
                                      : lastMessageDate.compareTo(snapshot.data.docs[k]['time'].toString()) == 0  ? Container() :
                                  (daysDifference2 - daysDifference) >= 1 ? Row(
                                    children: [
                                      Expanded(
                                        child: Divider(
                                  ...
                                  snapshot.data
                                      ...
                                      time:
                                      DateTime.parse(snapshot.data.docs[index]['time']+"Z").toLocal().minute > 10 ?
                                      DateTime.parse(snapshot.data.docs[index]['time']+"Z").toLocal().hour.toString()
                                          +":"+
                                          DateTime.parse(snapshot.data.docs[index]['time']+"Z").toLocal().minute.toString()
                                          :
                                      DateTime.parse(snapshot.data.docs[index]['time']+"Z").toLocal().hour.toString()
                                          +":"+
                                          "0"+DateTime.parse(snapshot.data.docs[index]['time']+"Z").toLocal().minute.toString(),
                                      image: imageLink,
                                      snapshot: snapshot,
                                      index: index
                                  )
                  ...
                  Container(
                      ...
                      child: ClipRRect(
                        ...
                        child: CachedNetworkImage(
                          ...
                          imageUrl: imageLink,
                          fit: BoxFit.cover,
                          placeholder: (context, string) => Container(height: 40,width: 40,),
                          errorWidget: (context, err ,f) => Icon(Icons.account_circle,size: 35,color: LIGHT_GREY_TEXT,),
                        ),
                      )
                  )
                ],
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text("Sending File", style: TextStyle(fontWeight: FontWeight.w600, color: LIGHT_GREY_TEXT),),
                          SizedBox(height: 5,),
                    ...
                    SizedBox(width: 10,),
                    Icon(Icons.cancel, color: LIGHT_GREY_TEXT,),
  ...
  Widget myMessageCard({String msg, String time, String image, snapshot, index}) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Row(
  ...
  void sendMessage(int type) async{
    ...
    if(isFirstMessage) {
      ...
      //     "last_msg": msg,
      //     "type": type,
      //     "messageCount": value.value == null ? 1 : value.value['messageCount'] == null ? 1 : value.value['messageCount'] + 1,
      //     "status": 0,
      //     "channelId": channelId
      //   });
      // });
  ...
  //           "last_msg": msg,
  //           "type": type,
  //           "messageCount": value.value == null ? 1 : value.value['messageCount'] + 1,
  //           "status": 0,
  //           "channelId": channelId
  //         });
  //       });
  ...
  void markAsSeen() async{
    ...
    seenRef = FirebaseDatabase.instance.reference().child(myUid.toString())
        .child('chatlist')
        .child(widget.uid.toString()).child("messageCount");
  ...
  checkSeenStatus() async {
    checkSeenRefListner = FirebaseDatabase.instance.reference().child(widget.uid)
        .child('chatlist')
        .child(myUid.toString()).child("messageCount")
  ...
  void pickFile() async{
    ...
    setState(() {
      file = f;
      print("path1================================"+f.files[0].path);
      print("path2================================"+file.files[0].path);
    });
    if(file.files[0].path.contains(".jpg")||file.files[0].path.contains(".png")||file.files[0].path.contains(".jpeg")){
      ...
      File _video = File(file.files[0].path);
      print("_video is====>>>>>${_video.path}");
      Uint8List u = await _video.readAsBytes();
      print("video is===>>$u");
      uploadFileToServer(u, "mp4", "file", u);
      // _pickVideo();
    }
  ...
  uploadFileToServer(Uint8List result, String extension, String type ,Uint8List thumbNail) async{
  ...
  Widget typeToWidget(int type, String message, String uid, String taskId, AsyncSnapshot<QuerySnapshot> snapshot, int index){
        ...
        child: Padding(
          ...
          child: Text(
            ...
            style: GoogleFonts.ubuntu(
                ...
                decoration: isURL(message) ? TextDecoration.underline : TextDecoration.none
            ),
          ),
        ),
      );
  ...
  //       video: path,
  //       thumbnailPath: (await getApplicationDocumentsDirectory()).path+ "/" + path,
              ...
              children: [
                ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    ...
                    fit: BoxFit.cover,
                    placeholder: (context, string) => Container(height: 40,width: 40,),
                    errorWidget: (context, err ,f) => Icon(Icons.account_circle,size: 50,color: LIGHT_GREY_TEXT,),
                  ),
                ),
                Container(
                  height: 50,
                ...
                Row(
                  children: [
                    Expanded(
                      child: InkWell(
                        onTap: (){
                          unsendMessage(snapshot.data.docs[index].id, index, snapshot.data.docs.length, snapshot);
                        },
                        child: Container(
  ...
  unsendMessage(String id,int index, int length, AsyncSnapshot<QuerySnapshot> snapshot) async{
    ...
    if(index > 0){
      ...
      await FirebaseFirestore.instance.collection("Chats").doc(channelId)
          .collection("All Chat")
          .doc(id).delete();
      await FirebaseDatabase.instance.reference().child(myUid)
          .child("chatlist")
          .child(widget.uid).remove();
      await FirebaseDatabase.instance.reference().child(widget.uid)
          .child("chatlist")
          .child(myUid).remove();
      Navigator.pop(context);
    }else{

      DatabaseReference documentReference = FirebaseDatabase.instance.reference().child(myUid).child('chatlist')
          .child(widget.uid);
      ...
      DatabaseReference documentReference2 = FirebaseDatabase.instance.reference().child(widget.uid).child('chatlist')
          .child(myUid);
  ...
  Future<Map<String, dynamic>> sendNotification(String userName, String message, String token) async {
